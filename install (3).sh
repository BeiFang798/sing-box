...
detect_ip() {
    ...
    if [ -n "${IPV4}" ] && [ -n "${IPV6}" ]; then
        ...
        echo "  [1] IPv4: ${IPV4}"
        echo "  [2] IPv6: ${IPV6}"
        # 强制从 /dev/tty 读取，避免 curl|bash 管道占用 stdin 导致 read 失败
        read -rp "请选择使用哪个 IP（输入 1 或 2，默认 1）: " ip_choice < /dev/tty
...
select_port() {
    echo ""
    info "请选择监听端口（直接回车使用默认 443）："
    echo "  常用选项：443（推荐，伪装性最佳）、8443、2096、2087"
    read -rp "请输入端口号 [默认 443]: " port_input < /dev/tty
    port_input="${port_input:-443}"

    # 校验：必须是纯数字且在有效范围内
    if ! [[ "${port_input}" =~ ^[0-9]+$ ]] || \
       [ "${port_input}" -lt 1 ] || [ "${port_input}" -gt 65535 ]; then
        warn "端口号无效（${port_input}），将使用默认端口 443"
        PORT=443
    else
        PORT="${port_input}"
    fi
